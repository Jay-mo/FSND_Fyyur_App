...
def search_venues():
  ...
  search_results = Venue.query.filter(Venue.name.ilike('%{}%'.format(search_term)))
  data = []
  count = 0
  num_upcoming_shows = 0
  for venue in search_results:
    ...
    #   if show.start_time > datetime.datetime.now():
    #     num_upcoming_shows += 1
    data.append({'id': venue.id, 'name': venue.name, "num_upcoming_shows": get_upcoming_shows(venue.shows) })
    count += 1
...
def search_artists():
  ...
  search_term=request.form.get('search_term', '')
  search_results = Artist.query.filter(Artist.name.ilike('%{}%'.format(search_term)))
  data = []
...
def edit_artist_submission(artist_id):
  ...
  try:
    ...
    db.session.rollback()
    flash('An error occurred. Artist ' + edit_artist.name + ' could not be edited.')
    print(sys.exc_info())
  finally:
    db.session.close()
...
def edit_venue_submission(venue_id):
  ...
  try:
    ...
    db.session.rollback()
    flash('An error occurred. venue ' + edit_venue.name + ' could not be edited.')
    print(sys.exc_info())
  finally:
    db.session.close()
